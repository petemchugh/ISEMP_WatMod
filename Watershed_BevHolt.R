          	...
          	for (i in 8:17) {
                  ...
                  for (g in 1:G) {
                          ...
                          NT[k,i-7,t,g] =  (CandN[g]*Rel_Surv[k, i,t,g]) / 
                            (1/(1+0*(Sr[k,2,t])) + 1/(C_ocean[k,i-7,t]) * 
...
#                         sum(CandN[]*Rel_Surv[k,i,t-1,]))	
      ...
      for (k in 1:K) {
            ...
            Rainbow.Male_GDist = 
            #(apply(RainbowSpawners_M[k,,t,],2,sum)/apply(RainbowSpawners_M[k,,t,],2,sum)) #Pete Nov 2015 -- beta
      ...
      for (k in 1:K) {
            ...
            N5_Imprint_F[k,4,t,2,k] = N5_Imprint_F[k,1,t+1,2,k] + 0.5*Hatch_Fish[k,5,t+1]
      ...
      for (k in 1:K) {
            for (i5 in 1:I5) {
                  for (g in 1:G) {
                        ...
                        ###***Pete May 2015 Addition***
                        Candidate_Smolt_Age[,i5,t,g]=CandN6_Imprint_F[,i5,g,]+CandN6_Imprint_M[,i5,g,]
                        Candidate_Smolt_Age[,i5,t,g]=Candidate_Smolt_Age[,i5,t,g]*Sr[k,6,t] #to get them into JDA equivalents
                        ###***Pete May 2015 Addition***
                  }
            }
      }
      ...
      for (k in 1:K) {
            for (g in 1:G) {
                  ...
                  sum(Rel_Surv[k,5,t,]*CandidateN6[k,g])) # +(Hatch_Fish[k,5,t]*(g==2))	
            }
      } # end of K
      ...
      if  (1==2) {
            ...
            for (k in 1:K) {
                  for (g in 1:G) {
                        #         for (k2 in 1:K) {
                        N_Imprint[k,6,t,g,] = temp[k,g,] * N[k,6,t,g]/ sum(temp[k,g,]) #+.0000000001)
                        N_Imprint_F[k,6,t,g,] = tempF[k,g,] * N[k,6,t,g]/ sum(temp[k,g,]) # +.0000000001)
                        N_Imprint[is.na(N_Imprint)]= 0
                        N_Imprint_F[is.na(N_Imprint_F)]= 0
                  }
                  #}
            ...
            saved.N_Imprint = N_Imprint
            Saved.N_IMprint_F = N_Imprint_F
